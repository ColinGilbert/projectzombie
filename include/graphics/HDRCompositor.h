...
public:
  ...
  virtual void
  compositorInstanceDestroyed(Ogre::CompositorInstance* destroyedInstance)
  {
...
public:
  ...
  enum TONEMAPPER
  {
    TM_NONE = 0, TM_LINEAR, TM_REINHARDS, TM_REINHARDSMOD, TM_LOG, TM_ADAPTLOG, TM_REINHARDLOCAL, TM_COUNT
  };
  ...
  enum STARTYPE
  {
    ST_NONE = 0, ST_PLUS, ST_CROSS, ST_PLUSCROSS
  };
...
public:
  ...
  float
  GetGlareStrength(void) const
  {
    return m_GlareStrength;
  }
  void
  SetGlareStrength(const float Strength)
  {
    m_GlareStrength = Strength;
  }

  int
  GetGlarePasses(void) const
  {
    return m_GlarePasses;
  }
  void
  ...
  float
  GetStarStrength(void) const
  {
    return m_StarStrength;
  }
  void
  SetStarStrength(const float Strength)
  {
    m_StarStrength = Strength;
  }

  int
  GetStarPasses(void) const
  {
    return m_StarPasses;
  }
  void
  ...
  bool
  GetLumAdaption(void) const
  {
    return m_LumAdaption;
  }
  void
  ...
  void
  SetKey(const float Key)
  {
    m_Key = Key;
  }

  float
  GetAdaptationScale(void) const
  {
    return m_AdaptationScale;
  }
  void
  SetAdaptationScale(const float Scale)
  {
    m_AdaptationScale = Scale;
  }
  ...
  void
  SetLocalE(const float e)
  {
    m_E = e;
  }

  float
  GetLocalPhi(void) const
  {
    return m_Phi;
  }
  void
  SetLocalPhi(const float phi)
  {
    m_Phi = phi;
  }

  void
  SetFrameTime(const float FrameTime)
  {
  ...
  virtual void
  notifyMaterialSetup(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat);
  ...
  virtual void
  notifyViewportSize(int width, int height);

  void
  DebugRendertarget(const int rt)
  {
...
private:
  ...
  BrightPass(void);
  void
  RenderDownSample(void);
  void
  CalculateLuminance(void);
  void
  BuildGlare(void);
  ...
  void
  BuildGaussGlare(float* out, float rho, float strength, float vpScale);

  void
  BuildGaussStarH(float* out, float rho, float strength);
